...
→ Show the user the available teams from the output, then:
  ...
  5. **REQUIRED — Do NOT skip this step.** Ask the user to pick a delivery mode using exactly this prompt:
     ...
     - Antigravity has no Monitor integration, so `monitor` and `both` modes are not offered here.
...
1. Parse the mode. Antigravity supports only `turn` and `off` — reject `monitor` and `both` with: "Antigravity only supports `turn` or `off` modes."
