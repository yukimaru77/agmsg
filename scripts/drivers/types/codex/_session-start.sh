...
# Defines agmsg_session_start. By default it returns to session-start.sh's
# common Monitor directive path; the legacy app-server bridge is used only when
# AGMSG_CODEX_BRIDGE=1.
#
# Normal Codex monitor mode matches Claude Code: SessionStart emits a Monitor
# directive and the host starts watch.sh. When launched through the old
# codex-monitor.sh wrapper, the TUI is attached to a shared app-server. In that
# compatibility path, hand the bridge off so incoming agmsg rows become turns in
# the current Codex thread without exposing socket/thread plumbing to the user.
