...
→ Show the user the available teams from the output, then:
  ...
  5. **REQUIRED — Do NOT skip this step.** Ask the user to pick a delivery mode using exactly this prompt:
     ...
     - Gemini CLI has no Monitor-tool equivalent, so `monitor` and `both` modes are not offered here.
...
3. Run: `~/.agents/skills/__SKILL_NAME__/scripts/join.sh <team> <agent_name> gemini "$(pwd)"`
...
1. Parse the mode. Gemini CLI supports only `turn` and `off` — reject `monitor` and `both` with: "Gemini CLI has no Monitor-tool equivalent; only `turn` or `off` modes are supported."
