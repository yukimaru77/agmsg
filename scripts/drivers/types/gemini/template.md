...
→ Show the user the available teams from the output, then:
  ...
  5. **REQUIRED — Do NOT skip this step.** Ask the user to pick a delivery mode using exactly this prompt:
     ...
     - This runtime has no Monitor tool, so `monitor` and `both` modes are not offered here.
...
5. Tell the user: "Now acting as `<name>`. Sends will use `<name>` as the from agent. (This runtime has no Monitor tool, so receive still covers all of your registered roles in this project.)"
...
1. Parse the mode. This runtime supports only `turn` and `off` — reject `monitor` and `both` with: "This runtime has no Monitor tool; only `turn` or `off` modes are supported."
