  ...
  mode)
    case "$#" in
      0)
        ...
        ;;
      1)
        # Platform-specific guard (not incidental type coupling): Codex monitor
        # delivery is unsupported by this Windows dispatcher, even though the
        # codex manifest lists 'monitor' for POSIX. delivery_modes is a global
        # fact and can't express "monitor everywhere except Windows", so this
        # one Windows dispatcher keeps an explicit early reject.
        if [ "$AGENT_TYPE" = "codex" ] && { [ "$1" = "monitor" ] || [ "$1" = "both" ]; }; then
          echo "Codex monitor delivery is not supported by the Windows dispatcher; only 'turn' or 'off' modes are supported." >&2
          exit 2
        fi
      ...
      *)
        if [ "$AGENT_TYPE" = "codex" ]; then
          echo "usage: agmsg mode [turn|off]" >&2
        else
